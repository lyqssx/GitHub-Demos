...
(function () {
  ...
  let deviceName = localStorage.getItem('air2-device-name') || 'Air 2';
...
function panel(place){var raw=(fit==='angle'||state.leakAdjusting)?1:fit==='battery'?4:fit==='ready'?5:6,list=raw===1?[['Wearing angle','adjust']]:raw===4?[['Wearing angle','done'],['Battery','checking']]:[['Wearing angle','done'],['Battery','done']],adjust=list.some(function(x){return x[1]==='adjust';}),starting=raw>=6;if(starting)return'<section class="r50-fit-panel r50-'+place+' is-starting" data-r50-place="'+place+'" role="status" aria-live="polite">'+skipButton()+'<div class="r50-start-ceremony"><i></i><strong>START</strong><span>Pumping begins now</span></div><div class="r50-progress"><i style="--r50-progress:100%"></i></div></section>';var copy=adjust?'<strong>Check pump alignment</strong>':raw>=5?'<strong>Everything looks good</strong>':'<strong>Fit Check</strong>';return'<section class="r50-fit-panel r50-'+place+'" data-r50-place="'+place+'" role="status" aria-live="polite"><header class="r50-fit-head"><div class="r50-fit-copy">'+copy+'</div>'+skipButton()+'</header><div class="r50-fit-checks">'+list.map(function(x){return'<span class="r50-fit-item is-'+x[1]+'"><i class="r50-check-icon">'+(x[1]==='done'?'✓':'<b></b>')+'</i>'+x[0]+'</span>';}).join('')+'</div>'+guide(adjust)+'<div class="r50-progress"><i style="--r50-progress:'+Math.min(100,Math.max(8,raw*20))+'%"></i></div></section>';}
  ...
  function fitOverlay() { return (state.leakAdjusting ? '<div class="pdcp-fit-backdrop"></div>' : '') + '<div class="pdcp-fit-overlay" role="dialog" aria-modal="true" aria-label="Pump alignment check">' + fitPanel() + '</div>'; }
  ...
  function deviceInfoPage() {
    const rows = [['Model','Air 2'],['SN','SN- DAC1101FDF49'],['MAC','SN- DAC1101FDF49'],['Firmware Version','46']];
    const edit = sheet === 'device-name' ? `<div class="pdcp-sheet-backdrop"><section class="pdcp-sheet" role="dialog" aria-modal="true" aria-label="Edit device name"><header><h2>Device name</h2>${btn('close-sheet','×','pdcp-icon','aria-label="Cancel editing"')}</header><form id="pdcp-device-name-form"><input name="deviceName" aria-label="Device name" value="${esc(deviceName)}" maxlength="40" required><button type="submit" class="pdcp-primary">Save</button></form></section></div>` : '';
    return `<section class="v4-screen pdcp-settings pdcp-device-info"><header class="v4-top">${btn('device-info-back','<img src="./assets/settings/back.svg" alt="">','v4-circle','aria-label="Back"')}<h1>Setting</h1></header><div class="pdcp-device-info-body"><div class="pdcp-info-pumps">${['L','R'].map(side=>`<div><div class="pdcp-info-pump ${side.toLowerCase()}"><img src="./assets/settings/info-pumps.png" alt="${side==='L'?'Left':'Right'} Air 2 pump"></div><span>${side}</span></div>`).join('')}</div><div class="pdcp-info-name"><strong>${esc(deviceName)}</strong>${btn('device-name','<img src="./assets/settings/edit.svg" alt="">','','aria-label="Edit device name"')}</div><dl class="pdcp-info-card">${rows.map(([label,value])=>`<div><dt>${label}</dt><dd><span>${value}</span>${['SN','MAC'].includes(label)?btn('copy-device-info','<img src="./assets/settings/copy.svg" alt="">','',`aria-label="Copy ${label}" data-value="${value}"`):''}</dd></div>`).join('')}</dl></div>${edit}</section>`;
  }
  ...
  function render(force = true) {
    ...
    root.innerHTML = report ? `<div class="pdcp-report-overlay"><div class="pdcp-report-underlay" inert aria-hidden="true">${v4Home()}</div><div class="pdcp-report-dim"></div>${reportCard()}</div>` : state.page === 'device' ? v4Device() : state.page === 'home' ? v4Home() : state.page === 'list' ? programList() : state.page === 'settings' ? settingsPage() : state.page === 'device-info' ? deviceInfoPage() : control();
  ...
  window.addEventListener('submit', e => {
    if (e.target.id === 'pdcp-device-name-form') {
      e.preventDefault(); e.stopImmediatePropagation();
      const name=e.target.elements.deviceName.value.trim(); if(!name) return;
      deviceName=name; localStorage.setItem('air2-device-name',name); sheet=null; render(); return;
    }
  ...
  window.addEventListener('click', e => {
    ...
    if (a === 'settings-info') {state.page='device-info';sheet=null;}
    if (a === 'device-info-back') {state.page='settings';sheet=null;}
    if (a === 'device-name') sheet='device-name';
    if (a === 'copy-device-info') {
      navigator.clipboard.writeText(value).then(()=>{el.setAttribute('aria-label','Copied');},()=>{el.setAttribute('aria-label','Copy failed, try again');}); return;
    }
    if (a === 'settings-firmware' || a === 'settings-reset' || a === 'settings-delete') sheet=a;
