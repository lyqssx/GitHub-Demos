...
function c32Vessel(side,amount){
  ...
  return `<div class="v4-pump c32-pump ${right?'right':''}"><span class="c32-vessel ${right?'c32-right':'c32-left'} c32-flow-${flow}"><img class="c32-base" src="./assets/figma-control-r32/vessel-${right?'r':'l'}-padded.svg" alt="Milk container"><span class="c32-liquid-clip"><span class="c32-liquid" style="--liquid-height:${level}px;height:${level}px"><svg viewBox="0 0 100 18" preserveAspectRatio="none" aria-hidden="true"><path d="M0 9 Q12 3 25 9 T50 9 T75 9 T100 9 V18 H0Z"/></svg><i></i></span>${c32Drops(flow)}</span></span><span class="amount">${label}</span></div>`;
}
