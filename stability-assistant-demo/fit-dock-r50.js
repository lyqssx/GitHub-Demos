...
window.v4RunFit=function(){clear();if(window.V3ProLeakFlow&&typeof window.V3ProLeakFlow.resetSession==='function')window.V3ProLeakFlow.resetSession();state.modal=null;state.fitAdjust=false;state.fitStage=0;state.running=true;state.paused=false;state.air2SessionEnded=false;state.air2ActiveSessionId='session-'+Date.now();state.air2LastPhysicsAt=Date.now();v4View();};
